...
specifically for the AROP (Automated Registration and Ortho-rectification Processing) step,
which is the second step in a four-step process. The script allows customization through
command-line arguments, including the agricultural year, tile names, and account information.
...
parser = argparse.ArgumentParser(description='Generate .slurm files for Analysis Ready Data production, AROP step 2 of 4')
...
arop_script_fullpath = os.path.join(homedir, 'tools/ard-ppln/ard-ppln_arop.py')
experiment_name = "AY" + str(ay)
dirname = "arop_" + experiment_name
...
ortho_fullpath = os.path.join(homedir, 'tools/arop')
...
for tile in tiles:
  ...
  # Options for the AROP script
  arop_options_list = ['--tile', tile, '--output', output_fullpath, '--reg', reg_fullpath, '--ortho', ortho_fullpath]
  arop_options = " ".join(arop_options_list)
  ...
  slurm_lines = [
      ...
      " ".join(["srun", python_fullpath, arop_script_fullpath, arop_options])
  ]
